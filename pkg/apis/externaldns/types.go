...
func (cfg *Config) ParseFlags(args []string) error {
	...
	app.Flag("create-service-record", "Always create a service-level record containing the A records for all pods, even if the hostname is set on the pod resource.").Default(strconv.FormatBool(defaultConfig.CreateServiceRecord)).BoolVar(&cfg.CreateServiceRecord)
